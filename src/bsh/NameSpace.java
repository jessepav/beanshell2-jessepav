...
/**
 * A namespace	in which methods, variables, and imports (class names) live. This is package public because
 * it is used in the implementation of some bsh commands.  However for normal use you should be using
 * methods on bsh.Interpreter to interact with your scripts.
 * <p/>
 * <p/>
 * A bsh.This object is a thin layer over a NameSpace that associates it with an Interpreter instance.
 * Together they comprise a Bsh scripted object context.
 * <p/>
 */
/*
...
public class NameSpace implements Serializable, BshClassManager.Listener, NameSource, Cloneable
{
    ...
    /**
     * The name of this namespace.  If the namespace is a method body namespace then this is the name of the
     * method.  If it's a class or class instance then it's the name of the class.
     */
    ...
    /**
     * The node associated with the creation of this namespace. This is used support getInvocationLine() and
     * getInvocationText().
     */
    ...
    /**
     * Note that the namespace is a class body or class instance namespace. This is used for controlling
     * static/object import precedence, etc.
     */
	/*
	...
	*/
    ...
    /**
     * Local class cache for classes resolved through this namespace using getClass() (taking into account
     * imports).  Only unqualified class names are cached here (those which might be imported).  Qualified
     * names are always absolute and are cached by BshClassManager.
     */
    ...
    /**
     * @param parent the parent namespace of this namespace.  Child namespaces inherit all variables and
    ...
    /**
     * Set the node associated with the creation of this namespace. This is used in debugging and to support
     * the getInvocationLine() and getInvocationText() methods.
     */
    ...
    /**
     * Set the variable through this namespace. This method obeys the LOCALSCOPING property to determine how
     * variables are set.
     * <p/>
     * Note: this method is primarily intended for use internally.  If you use this method outside of the
     * bsh package and wish to set variables with primitive values you will have to wrap them using
     * bsh.Primitive.
    ...
    /**
     * Set the value of a the variable 'name' through this namespace. The variable may be an existing or
     * non-existing variable. It may live in this namespace or in a parent namespace if recurse is true.
     * <p/>
     * Note: This method is not public and does *not* know about LOCALSCOPING. Its caller methods must set
     * recurse intelligently in all situations (perhaps based on LOCALSCOPING).
     * <p/>
     * <p/>
     * Note: this method is primarily intended for use internally.  If you use this method outside of the
     * bsh package and wish to set variables with primitive values you will have to wrap them using
    ...
    /**
     * Get the names of variables defined in this namespace. (This does not show variables in parent
     * namespaces).
     */
    ...
    /**
     * Get the names of methods declared in this namespace. (This does not include methods in parent
     * namespaces).
     */
    ...
    /**
     * Get the methods defined in this namespace. (This does not show methods in parent namespaces). Note:
     * This will probably be renamed getDeclaredMethods()
     */
    ...
    /**
     * Get the parent namespace. Note: this isn't quite the same as getSuper(). getSuper() returns 'this' if
     * we are at the root namespace.
     */
    ...
    /**
     * Get the top level namespace or this namespace if we are the top. Note: this method should probably
     * return type bsh.This to be consistent with getThis();
     */
    ...
    /**
     * A This object is a thin layer over a namespace, comprising a bsh object context.  It handles things
     * like the interface types the bsh object supports and aspects of method invocation on it.
     * <p/>
     * <p/>
     * The declaringInterpreter is here to support callbacks from Java through generated proxies.  The
     * scripted object "remembers" who created it for things like printing messages and other
...
// this?
     ...
     * <p/>
     * Note: this method is primarily intended for use internally.  If you use this method outside of the
     * bsh package you will have to use Primitive.unwrap() to get primitive values.
     ...
     * @param recurse If recurse is true then we recursively search through parent namespaces for the
     * variable.
     * <p/>
     * Note: this method is primarily intended for use internally.  If you use this method outside of the
     * bsh package you will have to use Primitive.unwrap() to get primitive values.
    ...
    */
    ...
    /**
     * Declare a variable in the local scope and set its initial value. Value may be null to indicate that
     * we would like the default value for the variable type. (e.g.  0 for integer types, null for object
     * types).  An existing typed variable may only be set to the same type. If an untyped variable of the
     * same name exists it will be overridden with the new typed var. The set will perform a
     * Types.getAssignableForm() on the value if necessary.
     * <p/>
     * <p/>
     * Note: this method is primarily intended for use internally.  If you use this method outside of the
     * bsh package and wish to set variables with primitive values you will have to wrap them using
	...
	*/
     ...
     * <p/>
     * Note: this method is primarily intended for use internally.  If you use this method outside of the
     * bsh package you will have to be familiar with BeanShell's use of the Primitive wrapper class.
     * @param declaredOnly if true then only methods declared directly in this namespace will be found and
     * no inherited or imported methods will be visible.
    ...
    /**
     * Import scripted or compiled BeanShell commands in the following package in the classpath.  You may
     * use either "/" path or "." package notation. e.g. importCommands("/bsh/commands") or
     * importCommands("bsh.commands") are equivalent.  If a relative path style specifier is used then it is
     ...
     * Commands are loaded from the classpath and may be imported using the importCommands() method.
     * <p/>
     * <p/>
     * This method searches the imported commands packages for a script or command object corresponding to
     * the name of the method.  If it is a script the script is sourced into this namespace and the
     * BshMethod for the requested signature is returned.  If it is a compiled class the class is returned.
     * (Compiled command classes implement static invoke() methods).
     * <p/>
     * <p/>
     * The imported packages are searched in reverse order, so that later imports take priority. Currently
     * only the first object (script or class) with the appropriate name is checked.  If another, overloaded
     * form, is located in another package it will not currently be found.  This could be fixed.
     * <p/>
     ...
     * @throws UtilEvalError if loadScriptedCommand throws UtilEvalError i.e. on errors loading a script
     * that was found
     */
     ...
     * @throws UtilEvalError on error in parsing the script or if the the method is not found after parsing
     * the script.
     */
	/*
	...
	*/
     ...
     * <p/>
     * <p/>
     * Check the cache first.  If an unqualified name look for imported class or package.  Else try to load
     * absolute name.
     * <p/>
     * <p/>
     * This method implements caching of unqualified names (normally imports). Qualified names are cached by
     * the BshClassManager. Unqualified absolute class names (e.g. unpackaged Foo) are cached too so that we
     * don't go searching through the imports for them each time.
     * @return null if not found.
     */
    ...
    /**
     * Try to make the name into an imported class. This method takes into account only imports (class or
     * package) found directly in this NameSpace (no parent chain).
     */
    private Class getImportedClassImpl(String name)
        ...
        if (fullname != null) {
			...
			*/
            ...
            else if (Interpreter.DEBUG) Interpreter.debug(
    ...
    */
    ...
    /**
     * Invoke a method in this namespace with the specified args and interpreter reference.  No caller
     * information or call stack is required.  The method will appear as if called externally from Java.
     * <p/>
     * @see bsh.This#invokeMethod(String methodName, Object [] args, Interpreter interpreter, CallStack
     * callstack, SimpleNode callerInfo, boolean)
     */
     ...
     * <p/>
     * @see This#invokeMethod(java.lang.String, java.lang.Object[], bsh.Interpreter, bsh.CallStack,
     * bsh.SimpleNode, boolean)
     */
    public Object invokeMethod(
        String methodName, Object[] args, Interpreter interpreter,
    ...
    /**
     * This is the factory for Name objects which resolve names within this namespace (e.g. toObject(),
     * toClass(), toLHS()).
     * <p/>
     * <p/>
     * This was intended to support name resolver caching, allowing Name objects to cache info about the
     * resolution of names for performance reasons.  However this not proven useful yet.
     * <p/>
     * <p/>
     * We'll leave the caching as it will at least minimize Name object creation.
     * <p/>
     * <p/>
     * (This method would be called getName() if it weren't already used for the simple name of the
     * NameSpace)
     * <p/>
     * <p/>
     ...
     * <p/>
     * This method is in NameSpace for convenience (you don't have to import bsh.ClassIdentifier to use it
     * );
     */
    ...
    /**
     * Clear all variables, methods, and imports from this namespace. If this namespace is the root, it will
     * be reset to the default imports.
    ...
    /**
     * Import a compiled Java object's methods and variables into this namespace.  When no scripted method /
     * command or variable is found locally in this namespace method / fields of the object will be checked.
     * Objects are checked in the order of import with later imports taking precedence.
     * <p/>
     */
	/*
